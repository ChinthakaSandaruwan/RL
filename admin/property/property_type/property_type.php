    ...
    if (!isset($_POST['csrf_token']) || $_SESSION['csrf_token'] !== $_POST['csrf_token']) {
        ...
        if ($action === 'add') {
            if (empty($typeName)) {
                ...
                // Check if exists
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM property_type WHERE type_name = ?");
                $stmt->execute([$typeName]);
                if ($stmt->fetchColumn() > 0) {
                    $_SESSION['_flash']['error'] = "Property type already exists.";
                } else {
                    $stmt = $pdo->prepare("INSERT INTO property_type (type_name) VALUES (?)");
                    if ($stmt->execute([$typeName])) {
                        $_SESSION['_flash']['success'] = "Property type added successfully.";
                    } else {
                        $_SESSION['_flash']['error'] = "Failed to add property type.";
                    }
                }
            }
            ...
            if (empty($typeName) || $typeId <= 0) {
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM property_type WHERE type_name = ? AND type_id != ?");
                $stmt->execute([$typeName, $typeId]);
                if ($stmt->fetchColumn() > 0) {
                    $_SESSION['_flash']['error'] = "Property type name already exists.";
                } else {
                    $stmt = $pdo->prepare("UPDATE property_type SET type_name = ? WHERE type_id = ?");
                    if ($stmt->execute([$typeName, $typeId])) {
                        $_SESSION['_flash']['success'] = "Property type updated successfully.";
                    } else {
                        $_SESSION['_flash']['error'] = "Failed to update property type.";
                    }
                }
            }
            ...
            if ($typeId <= 0) {
                ...
                // Check usage
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM property WHERE property_type_id = ?");
                $stmt->execute([$typeId]);
                $count = $stmt->fetchColumn();

                if ($count > 0) {
                    $_SESSION['_flash']['error'] = "Cannot delete this type. It is currently assigned to $count propertie(s).";
                } else {
                    $stmt = $pdo->prepare("DELETE FROM property_type WHERE type_id = ?");
                    if ($stmt->execute([$typeId])) {
                        $_SESSION['_flash']['success'] = "Property type deleted successfully.";
                    } else {
                        $_SESSION['_flash']['error'] = "Failed to delete property type.";
                    }
                }
            }
        }
...
$query = "
    SELECT pt.*, COUNT(p.property_id) as usage_count 
...
<head>
    ...
    <link rel="stylesheet" href="../../css/admin.css"> <!-- Assuming general admin css exists -->
    <link rel="stylesheet" href="property_type.css">
</head>
<body>
    ...
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <h2 class="fw-bold mb-0 text-dark">Manage Property Types</h2>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title fw-bold" id="modalTitle">Add Property Type</h5>
                ...
                <div class="modal-body">
                    <form id="typeForm" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="type_name" id="typeName" class="form-control form-control-lg" placeholder="e.g. Apartment, Villa" required>
                        </div>
    ...
    <script src="property_type.js"></script>
</body>
</html>
