...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_action'])) {
    ...
    try {
        ...
        $check = $pdo->prepare("
            ...
            WHERE r.rent_id = ? AND rm.owner_id = ?
        ");
        $check->execute([$rentId, $user['user_id']]);
        
        if ($check->rowCount() > 0) {
            $upd = $pdo->prepare("UPDATE room_rent SET status_id = ? WHERE rent_id = ?");
...
$sql = "
    SELECT r.*, rm.title, rm.room_code, u.name as customer_name, u.mobile_number, u.email, rs.status_name
    FROM room_rent r
    JOIN room rm ON r.room_id = rm.room_id
    JOIN user u ON r.customer_id = u.user_id
    JOIN rent_status rs ON r.status_id = rs.status_id
    WHERE rm.owner_id = ? AND $statusCondition
...
<head>
    <meta charset="UTF-8">
    <title>Room Rent Approval - Rental Lanka</title>
...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark">Rent Approvals <span class="text-muted fs-4">| Room</span></h2>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td class="ps-4">
                                    ...
                                    <small class="text-muted"><?= htmlspecialchars($req['room_code']) ?></small>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="small">
                                        <div class="mb-1"><i class="bi bi-calendar-check me-1"></i> <?= date('M d', strtotime($req['checkin_date'])) ?> - <?= date('M d, Y', strtotime($req['checkout_date'])) ?></div>
                                        <div><i class="bi bi-people me-1"></i> Guests: <?= $req['guests'] ?></div>
                                    </div>
                                </td>
                                <td>
