...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_action'])) {
    ...
    try {
        ...
        $newStatus = ($action === 'approve') ? 1 : 3; // 1=Booked, 3=Cancelled

        // Verify ownership and valid Rent ID
        $check = $pdo->prepare("
            SELECT r.rent_id 
            FROM property_rent r 
            JOIN property p ON r.property_id = p.property_id 
            WHERE r.rent_id = ? AND p.owner_id = ?
        ");
        $check->execute([$rentId, $user['user_id']]);
        
        if ($check->rowCount() > 0) {
            $upd = $pdo->prepare("UPDATE property_rent SET status_id = ? WHERE rent_id = ?");
...
$sql = "
    SELECT r.*, p.title, p.property_code, u.name as customer_name, u.mobile_number, u.email, rs.status_name
    FROM property_rent r
    JOIN property p ON r.property_id = p.property_id
    JOIN user u ON r.customer_id = u.user_id
    JOIN rent_status rs ON r.status_id = rs.status_id
    WHERE p.owner_id = ? AND $statusCondition
...
<head>
    <meta charset="UTF-8">
    <title>Property Rent Approval - Rental Lanka</title>
...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold text-dark">Rent Approvals <span class="text-muted fs-4">| Property</span></h2>
                        ...
                        <?php else: ?>
                            ...
                            <tr>
                                <td class="ps-4">
                                    ...
                                    <small class="text-muted"><?= htmlspecialchars($req['property_code']) ?></small>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="small">Payment: LKR <?= number_format($req['price_per_month'], 2) ?> / Month</div>
                                </td>
                                <td>
                                    <?php 
