...
if ($isEnabled) {
    // Divali content validation logic (if specific structure needed)
    echo '<style>.divali-container { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 9999; pointer-events: none; }</style>';
    echo '<div class="divali-container">';
    ?>
    <!-- Original Divali Animation Content -->
    <!-- SVG Spritesheet -->
    ...
    <div class="container d-none">
        <!-- Hidden container to satisfy JS selectors if needed, or we adapt JS -->
    </div>
    
    <!-- Using iframe or direct include might be safer for complex CSS/JS isolation, 
         but here we assume Global Include. 
         Ideally, we should link the CSS/JS. -->
    <link rel="stylesheet" href="<?= app_url('super_admin/special_holidays/divali/animation.css') ?>">
    <script src="<?= app_url('super_admin/special_holidays/divali/animation.js') ?>"></script>
    
    <?php
    echo '</div>'; // End divali-container
}
?>